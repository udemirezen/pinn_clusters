...
In our study, we investigated the feasbility of a physics-informed neural network to invert for ice-shelf hardness spatial profile $B(x)$ given noisy training data of only velocity $u(x)$  and thickness $h(x)$. We introduced a weighting hyperparameter $\gamma$ in the objective function of the PINN, adjusting the relative weighting between equation and data loss.  
