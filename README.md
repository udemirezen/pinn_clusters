...
Please direct questions about this code and documentation to Yunona Iwasaki: yiwasaki@berkeley.edu .
## Overview
...
<p align="center">
Figure 2. Distribution of $B_{err}$, $u_{err}$, and $h_{err}$ using collocation resampling for constant $B(x)$ profile over 501 trials with noise = 0.3. We compare the trial error distributions for values of  $\frac{\gamma}{1-\gamma} < 1.$ _No clustering is observed for these values of $\gamma$ using initial training with collocation resampling._ In grey, we overlay the prediction errors from all PINN solutions obtained with $\frac{\gamma}{1-\gamma} < 1$ using the original training scheme (4 $\gamma$-values * 501 trials  = 2005 samples).
</p>
